 ...
 * selected-radio state, the selected-pinned-first ordering, and the exact-Country dispatch on tap.
 */
class RegionPickerSheetTest {
    ...
    @Test
    fun selectedCountryIsPinnedFirst() {
        setContent()

        // First radio-button row in the list is the selected one (selected pinned to top).
        composeTestRule.onAllNodes(isRadio).onFirst().assert(hasText(UK.name) and isSelected())
    }
