...
| 6 — Cleanup & hardening | #238–#240 | 🔶 PARTIAL — 6.1 done (#238 closed); 6.2 CSPRNG done, encrypted-storage half Mac-blocked; 6.3 docs+Kover done, baseline regen blocked by a mis-wired `stabilityDump` task (analyzer itself works) |
...
- Stability tooling note: the `stabilityDump`/`stabilityCheck` tasks report "No composables found" and skip **only when forced via `--rerun-tasks`** (which makes the dump read an empty analyzer pass); a *normal* compile actually emits correct per-module composable data to `build/stability/<compilation>/stability-info.json`. The committed `*.stability` files are stale legacy artifacts and the dump task is mis-wired (no output/compile-dep) so it never refreshes them — see the #240 (6.3) entry for the full diagnosis. `:feature:deals` has no `stability/` reference and CI's `stabilityCheck` still passes.
...
- **#239 (6.2) — PARTIAL (CSPRNG done; encrypted storage still open).**
  - **CSPRNG — DONE.** PKCE verifier + CSRF `state` now draw from the platform CSPRNG via an `expect/actual secureRandomBytes(size)` (`SecureRandom.kt` in `auth/oauth`): `java.security.SecureRandom` on Android, `SecRandomCopyBytes` (Security framework) on iOS, replacing `Random.Default`. Android/common verified by `:remote:itad:testAndroidHostTest` (added `randomState`/`secureRandomBytes` tests); the iOS actual is standard K/N Security interop, **Mac compile-verified 2026-06-24** (`:iosApp:compileKotlinIosSimulatorArm64` green + full Xcode build/boot).
  - **Encrypted token storage — DONE (build-verified 2026-06-24).** `AuthTokenStore` is bound to the `SECURE_QUALIFIER` `Storage` (`DomainModule.kt`), backed by an **Android Keystore AES/GCM** value-encrypting backend (`EncryptedSharedPreferencesBackend` — the recommended Keystore approach, *not* the deprecated `security-crypto` lib) and an **iOS Keychain** backend (`KeychainBackend`, `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly`). The iOS half is now Mac compile-verified (`:iosApp:compileKotlinIosSimulatorArm64` + Xcode build/boot). **Residual:** runtime round-trip (a token is only written on login) rides on the deferred live OAuth smoke test.
- **#240 (6.3) — PARTIAL/OPEN.** Docs/handover kept current each phase ✅; root Kover includes every new epic module (`:feature:account`, `:feature:deals`, `:remote:itad`) ✅ (`:remote:igdb`'s absence is a pre-existing, unrelated omission). **Stability baselines NOT regenerated — but NOT because the analyzer is broken here.** Investigated 2026-06-09: the skydoves stability **analyzer works fine on this Linux box** — a normal `compileAndroidMain` emits a correct, *current* `build/stability/compileAndroidMain/stability-info.json` (15 composables; iOS-sim 16), reflecting the post-redesign composable set, not the stale one. The blocker is the **`stabilityDump` Gradle task (analyzer 0.7.5) being mis-wired**: it declares **no task output** (so deleting a `*.stability` baseline doesn't mark it out-of-date → the task is always `UP-TO-DATE` and won't rewrite) and **no dependency on the compile** (so `--rerun-tasks` forces it to read an empty analyzer pass → `ℹ️ No composables found … skipping`). Net: the turnkey regen task can't refresh the baselines as-wired, even though the correct data exists in `build/stability/.../stability-info.json`. The committed `*.stability` files stay stale-but-`stabilityCheck`-passing (the check compares stability *classifications*, not param names). Options to actually unblock (deferred tracked debt): (a) fix the task wiring — declare the `.stability` file as output + depend on the matching compile (touches `build-logic` + the AGP/Kotlin/KSP/analyzer lockstep); (b) script a `stability-info.json → .stability` converter (hand-rolls the plugin's serializer — fragile vs. `stabilityCheck`); (c) bump/patch the analyzer upstream.

## Remaining epic-wide verification (needs a Mac + real OAuth client id)
Carried since Phase 2:
- **DONE (2026-06-24):** `:iosApp:compileKotlinIosSimulatorArm64` compiles all the `iosMain` changes (OAuth launcher + every NavHost/Koin edit across Phases 1–5) — green, plus a full Xcode Debug build (`BUILD SUCCEEDED`) and a clean simulator boot on macOS (Xcode 26.5, iPhone 17 / iOS 26.5 arm64 sim).
- **Still deferred** (needs ITAD redirect registration + live API): Live OAuth smoke test: fill `local.properties itadOauthClientId` / `Secrets.xcconfig ITAD_OAUTH_CLIENT_ID`, register the redirect `pm.bam.gamedeals://oauth/itad` with ITAD, then login → `/user/info` → username; add/remove waitlist; force a 401 → silent refresh; logout. Confirm the `ItadOAuthConfig` URLs + the `obj.game` PUT/DELETE body shapes against the live API.

## Dead-code cleanup (post-Phase-5)
- **DONE** (commit below): removed 10 unused `:feature:home` strings left by Phases 3–5 — the favourites strings (`home_screen_favourites_label`, `home_screen_all_favourites_label`, `home_screen_floating_favourites_icon`, `home_screen_floating_settings_icon`, `home_screen_favourite_indicator`, `home_screen_floating_search_icon`, `home_screen_floating_search_error_icon`) and dead store-strip strings (`home_screen_store_banner`, `home_screen_all_store_deals_label`, `home_screen_loading_label`). Kept `home_screen_store_deal_row_description*` (reused by the Trending deal rows; `_favourite` text updated "favourited"→"waitlisted").
- **`:feature:store` — DECISION: keep, do not delete.** Verified genuinely unreachable (`NavigationActions.navigateToStore` is never called; only the `storeScreen()` route registration + `Destination.Store` remain). It's a complete, working single-store deals view that the **`onBrowseStores` overflow item is designed to reach** — deleting it and later rebuilding for that would be wasteful. Future small task: wire `onBrowseStores` → a stores-list picker → the existing Store screen. (`navigateToStore`/`Destination.Store`/`storeScreen()` stay wired meanwhile; they're unused-but-harmless.)
- Still deferred: the `favourite*`-named locals/keys in Store/Search (cosmetic only — those strings/markers are still in use, just named "favourite"); fold the **Giveaways tab** into the shell top bar (Phase 1 interim — it keeps its own top bar, so Search isn't reachable on that tab).
